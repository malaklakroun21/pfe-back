...
describe('session + credit + rating flow', () => {
  ...
  it('runs complete session-credit-rating lifecycle', async () => {
    ...
    expect(teacherAfter.xpTotal).toBe(20);
    expect(teacherAfter.level).toBe(1);
    expect(teacherAfter.xpHistory).toHaveLength(1);
    expect(teacherAfter.xpHistory[0]).toMatchObject({
      amount: 20,
